...
// 工作区投影同步重试参数（globalPrompt.workspaceSyncRetryMax / workspaceSyncRetryIntervalMs 可配置）
let wsSyncRetryMax = 40
let wsSyncRetryIntervalMs = 500
...
function syncWorkspaceProjection(ctx, wsScope, activeScope) {
  ...
  activeScope.update({ active }).catch((e) => console.warn('[dsh-global-prompt] scope update failed:', e && e.message ? e.message : e))
  if (changed) {
...
async function syncWithRetry(ctx, wsScope, activeScope, attempt) {
  const MAX = wsSyncRetryMax // 上限可由 globalPrompt.workspaceSyncRetryMax 配置
  let ok = false
  try {
  ...
  if (ok) return
  if (attempt < MAX) {
    ctx.timeout(() => { syncWithRetry(ctx, wsScope, activeScope, attempt + 1) }, wsSyncRetryIntervalMs)
  }
}
...
export function apply(ctx, cfg) {
  ...
  wsSyncRetryMax = (cfg && typeof cfg.workspaceSyncRetryMax === 'number') ? cfg.workspaceSyncRetryMax : 40
  wsSyncRetryIntervalMs = (cfg && typeof cfg.workspaceSyncRetryIntervalMs === 'number') ? cfg.workspaceSyncRetryIntervalMs : 500
  ...
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      fsStatusScope.update({ byScope: { global: statuses } }).catch((e) => console.warn('[dsh-global-prompt] scope update failed:', e && e.message ? e.message : e))
  ...
  ctx.effect(() => ctx.systemPrompt.section({
    ...
    text: (context) => {
      ...
      fsStatusScope.update({ byScope: { [best.pathKey]: statuses } }).catch((e) => console.warn('[dsh-global-prompt] scope update failed:', e && e.message ? e.message : e))
