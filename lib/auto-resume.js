...
export function apply(ctx, cfg) {
  ...
  // sessionPersistence 为可选服务：缺失时 settings 注册仍然正常，仅启动恢复降级跳过（不拖垮整包）
   ...
   * - seedLength !== 0：非空白会话（无用户消息的空白会话无需恢复；seedLength 缺失视为可恢复）。
   */
  ...
  async function restoreAll() {
    try {
      // sessionPersistence 缺失（无会话持久化）→ 无法枚举历史会话，启动恢复静默跳过
      if (!persistence) { console.warn('[dsh-auto-resume] sessionPersistence unavailable, startup restore skipped'); return; }
