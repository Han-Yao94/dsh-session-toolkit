...
function registerTools(ctx, agents, tools) {
  ...
  const sendTool = defineTool({
    ...
    parameters: {
      ...
      wakeup: { type: 'boolean', description: '是否投递并唤醒，默认 true。true：目标正在执行 ⇒ 进目标当前轮的下一个步边界（不新开一轮，也不打断当前步）；目标空闲 ⇒ 立刻开一轮（此时本就没有步边界可插，消息在轮首被收下）。⚠️ 两种情形仍会排队：㈠ 目标上一个活动正在被取消（内核 wakingAfterAbort 把唤醒型输入降级为下一轮）；㈡ 目标空闲时投递——没有步边界可插，只能等一个轮次边界。故本参数**不承诺"不排队"**。false：只投递不唤醒（执行中的目标在下一个步边界读到；空闲的目标留到它下次被唤醒时）' }
    },
    output: {
      schema: { type: 'json' },
      render(args, value) {
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        // **仍会排队的情形一：wakingAfterAbort**（内核有意行为，不是缺陷）。`send()` 里有重分类
        // （packages/core/agent-loop/src/agent.ts:153-159）——
        //     const wakingAfterAbort = wakeup && this.phase.kind !== 'idle' && this.phase.abort.signal.aborted
        //     const resolvedTarget = wakingAfterAbort ? 'next-turn' : target
        // ⇒ 目标上一个活动**正在被取消**时，steer 会静默降级为 `next-turn`，真等下一轮（内核注释：
        // Waking input cannot join an aborted activity）。**别把 steer 当成"一定插进当前轮"的契约来依赖。**
        // **仍会排队的情形二：目标空闲时投递**。空闲目标**没有步边界可插** ⇒ 消息只能在**下一个轮次
        // 边界**被 `claim` 收下（内核 inbox.ts:109-114 的 claim 会先抽干 next-step，故它优先级高于
        // next-turn）。⇒ 此时从**消费时机**看仍是"排队"。
        // ⚠️ **`target` 是声明，不是行为**（实测反例：peer-mufc7pen-8otia2xlb6，投递时有轮在跑、
        //    等待 3 ms，尺子 scripts/inbox-queue.verify.mjs 仍判 queued）。⇒ **只看
        //    `target === 'next-step'` 会把「空闲投递」那一格误读成"插进正在执行的轮"**；判断插入必须看
        //    投递→消费之间**有没有新的 turn/start**（0 次才是 inserted-live）。
