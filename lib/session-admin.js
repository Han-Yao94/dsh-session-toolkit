...
function registerTools(ctx, agents, tools) {
  ...
  async function createSession(args) {
    ...
    const result = {
      ...
      title: null,
      // 分组状态（裁定 #5 引入，裁定 #6 §30.3 改为**显式枚举**）。两个字段各答一个问题：
      //  · `workspaceGrouping`：本次挂载**做了什么**——'attached' | 'not-found' | 'unavailable' | 'resolve-failed' | 'attach-failed'（五态机器可判）；
      //  · `workspaceId`：**仅当 grouping === 'attached'** 时为该 workspace id，**其余一律 null**。
      // 拆开的原因：`null` 有四种来源，若只留一个可空 id，调用方就只能读 notes 散文来区分
      // 「未命中（正常）」与「环境坏了（异常）」——而那两种情况隐含的下一步动作相反。
      workspaceGrouping: 'not-found',
      workspaceId: null,
      // preset.note 只在「legit 降级」出现（服务缺失）；resolve 失败走的是 ok:false 早退，不会到这里。
      notes: preset.note === undefined ? [] : [preset.note],
    }

    // 挂载 workspace（裁定 #5）：**必须在会话建好之后**——attachSession 要读会话 header 的 cwd 做一致性校验
    // （entity.ts:109-140），没有会话就无从校验。挂载失败**不回滚会话**（会话是真实资产）。
    const attach = await attachWorkspaceToSession(result.sessionId, cwd)
    result.workspaceGrouping = attach.grouping
    if (attach.id !== null) result.workspaceId = attach.id
    if (attach.note !== undefined) result.notes.push(attach.note)
  ...
  // 把新建会话挂进「cwd 对应的、**已存在**的」workspace（裁定 #5；返回形状按裁定 #6 改为显式枚举）。
  //
  // 为什么必须用枚举而不是「一个可空的 id」（裁定 #6 §30.3）：`null` 有**四种**来源——
  // 服务缺失 / 解析抛错 / 未命中 / 挂载失败——**字段值全同，区分信息只能落在 notes 的散文里**。
  // 而「字段必须能回答它名字所问的问题」：`workspaceId` 回答不了「该会话属于哪个 workspace」
  // （那要读回 registry），只能回答「本次是否挂上」⇒ 于是拆成两个字段各答一个问题：
  //   · `workspaceGrouping`（枚举）：本次**做了什么**——四态机器可判；
  //   · `workspaceId`（string|null）：**仅 attached 时**有值，其余一律 null（并在 description 写明前提）。
  //
  // 四态与各自含义：
  //  · 'attached'       挂载成功 ⇒ workspaceId = 该 workspace id
  //  · 'not-found'      `resolveByPath` **成功返回但未命中** ⇒ **静默**（未分组是合法结果，AC27；不创建新 workspace）
  //  · 'unavailable'    `workspaceRegistry` 服务不可用 ⇒ 合法降级，带说明性 note
  //  · 'resolve-failed' `resolveByPath` **抛错**（cwd 无法规范化）⇒ 带 note，保留内核原文
  //  · 'attach-failed'  `attachSession` **抛错**（已解析到 workspace，但挂不上）⇒ 带 note，保留内核原文
  //
  // 为什么把「解析失败」与「挂载失败」拆成两个态（裁定 #7 §32.3 ②，D 提出）：
  // 按「字段必须能回答它名字所问的问题」，这两条**本次做的事不同**——
  // 前者的下一步是「去登记/创建工作区」，后者是「去查为什么挂不上」，**隐含动作相反**。
  // （另一条被放弃的候选：保留单一 'failed' 但在挂载失败时给出已解析到的 id——它需要一个新的字段语义，
  //   而拆态只需改一个枚举值 ⇒ 记为「将来可恢复」的替代方案，若确需该 id 可另加 `workspaceResolvedId`。）
  //
  // 硬约束（§28.2）：不复用 `workspace.create()`——往用户左侧导航**加一条新工作区**是对外可见副作用，
  // 官方路径的 workspace 来自请求里显式给的 workspaceId，而本工具只有 cwd；静默创建属越权。
  // 挂载用 `ctx.get('workspaceRegistry')` **调用时读取**（与 lib/peer-message.js:78 同法），不硬 inject。
  async function attachWorkspaceToSession(sessionId, cwd) {
    const workspaceRegistry = ctx.get('workspaceRegistry')
    if (workspaceRegistry === undefined) {
      return {
        grouping: 'unavailable',
        id: null,
        note: 'workspaceRegistry 服务不可用：未挂载 workspace（会话仍会创建，落在「未分组」）',
      }
    }
    let workspace
    try {
      ...
      return {
        grouping: 'resolve-failed',
        id: null,
        // 可选措辞收紧：原文的括号容易被读成「该路径没被登记为 workspace」（= not-found），
        // 但这两者含义差一个量级（环境坏 vs 正常结果），故写明它其实是「无法解析该路径」。
        note: 'workspace 解析失败：该 cwd 无法被解析/规范化为一个已有目录（这不等于「未登记为 workspace」）：' + describe(error),
      }
    }
    if (workspace === undefined) {
      // 未命中：不创建新 workspace（硬约束 2），保持未分组，且**不加 note**（AC27）。
      return { grouping: 'not-found', id: null }
    }
    try {
      await workspace.attachSession(sessionId)
    } catch (error) {
      return {
        grouping: 'attach-failed',
        id: null,
        note: '会话已创建，但挂载到 workspace 失败（会话仍在「未分组」；该 workspace 已解析到，问题出在挂载这一跳）：' + describe(error),
      }
    }
    return { grouping: 'attached', id: String(workspace.id) }
  }

  const createTool = defineTool({
    name: 'create_session',
    description: '自主创建一个新的顶层会话（GUI 左侧导航里的一个聊天窗口）。cwd 与 prompt 均必填：cwd 必须是绝对路径（无 cwd 的会话不会进宿主列表），prompt 是新会话的首条消息。创建成功即产生一条真实用户消息（会真实跑一轮模型、消耗一次调用）；按内核设计，产生过事件（消息）的会话会被持久化，本工具因此不再提供「只登记、不说话」的临时会话。**若 cwd 命中一个已登记的工作区，会话会被挂到它下面，否则留在「未分组」——本工具不会新建工作区。** 返回体的 workspaceGrouping 说明本次分组结果：attached（已挂上）/ not-found（没有匹配的已登记工作区，正常结果）/ unavailable（workspace 服务不可用）/ resolve-failed（cwd 无法解析为已有目录）/ attach-failed（已解析到工作区但挂载失败）。**workspaceId 只在 workspaceGrouping === "attached" 时才有值**，其余四种状态一律为 null。是否真正开跑、prompt 是否已落盘，均未在本工具内验证。可选 title 立即设定标题（会 pin 住，不被自动标题覆盖）。',
    parameters: {
