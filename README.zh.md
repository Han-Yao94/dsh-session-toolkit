...
已发布至 **npm**（`dsh-session-toolkit`，v0.1.4（示例，发布前请与 package.json 版本号核对一致），MIT）并同步至 **GitHub**（`github.com/Han-Yao94/dsh-session-toolkit`）。纯 JS 包——**无构建步骤、无 prepare 脚本**。`files` 已白名单 `lib/`、`client/`、`cordis.patch.yml` 与 README。
